...
# 将 test_GoogLe accuracy 保存到 "tensorboard/test_GoogLe" 文件夹
log_dir = os.path.join('tensorboard', 'test_google')
...
def main():
    ...
    with open("file/calss_indices.json", 'w') as json_file:
    ...
    net = GoogLeNet(num_classes=2, aux_logits=True, init_weights=True)  # 实例化网络(5分类)
    net.to(device)
    loss_function = nn.CrossEntropyLoss()
    optimizer = optim.Adam(net.parameters(), lr=0.0001)

    epochs = 100
    save_path = "Offline/GoogLeNet_GPU.pth"
    best_accuracy = 0.0
    ...
    for epoch in range(epochs):
        ...
        test_writer.add_scalar('Accuracy', val_accuracy, epoch)
        test_writer.add_scalar('loss', running_loss / train_steps, epoch)
